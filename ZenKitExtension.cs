...
namespace GothicGodot
{
    public static class ZenKitExtension
    {
        ...
        public static (List<int>, List<float>) ToBoneWeight(this List<SoftSkinWeightEntry> weights, List<int> nodeMapping)
        {
            if (weights == null)
                throw new ArgumentNullException("Weights are null.");
    
            var indices = new List<int>();
            var weightsData = new List<float>();

            if (weights.Count < 1 || weights.Count > 4)
            ...
            for (var i = 0; i < weights.Count; i++)
            {
                ...
                indices.Add(index);
                weightsData.Add(weights[i].Weight);
            }
