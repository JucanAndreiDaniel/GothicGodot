...
public static class WayNetHelper
{
    ...
    public static List<FreePoint> FindFreePointsWithName(Vector3 lookupPosition, string namePart, float maxDistance)
    {
        var matchingFreePoints = GameData.FreePoints
            ...
            .Where(pair => lookupPosition.DistanceTo(pair.Value.Position) <= maxDistance) // PF is in range
            .OrderBy(pair => lookupPosition.DistanceTo(pair.Value.Position)) // order from nearest to farthest
    ...
    {
        var wayPoint = GameData.WayPoints
            .OrderBy(pair => pair.Value.Position.DistanceTo(lookupPosition))
    ...
    public static FreePoint FindNearestFreePoint(Vector3 lookupPosition, string fpNamePart)
    {
        return GameData.FreePoints
            .Where(pair => pair.Value.Name.Contains(fpNamePart))
            .OrderBy(pair => pair.Value.Position.DistanceTo(lookupPosition))
            .Select(pair => pair.Value)
            .FirstOrDefault();
    }
    ...
    private class PriorityQueue
    {
        ...
        public void Remove(DijkstraWaypoint waypoint)
        {
            ...
            if (index == -1)
            {
                ...
                GD.Print("The specified waypoint " + waypoint.Name + " is not in the queue.");
                return;
            }
